...
try {
    ...
    foreach (['GET /geographic-locations','GET /unit-types/1/configurations'] as $forbidden) { [$method,$uri] = explode(' ', $forbidden, 2); $request = new Request(new InputBag([]),new InputBag([]),new InputBag([]),new HeaderBag(['Authorization'=>'Bearer viewer']),new InputBag([]),new InputBag([]),[],$method,$uri); bf014ReadAssert(bf014ReadResult($router->dispatch($method,$uri,$request))['status']===404, "{$forbidden} must not be registered by B1/B2."); }
