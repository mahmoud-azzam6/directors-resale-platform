...
try {
    ...
    $verify = function () use ($catalog, $pdo, $measurementCodes, $attributeTypes, $options, $one, $two, $three, $four, $five): void {
        ...
        b2check(array_column($pdo->query('SELECT seed_key FROM property_catalog_seed_versions ORDER BY id')->fetchAll(PDO::FETCH_ASSOC), 'seed_key') === [$one->key,$two->key,$three->key,$four->key,$five->key], 'Ledger order');
    };
    ...
    b2check($cli() === $applied, 'CLI first run'); $verify(); b2check($cli() === $skipped, 'CLI second run');
