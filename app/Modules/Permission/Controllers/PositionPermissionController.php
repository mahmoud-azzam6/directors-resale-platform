...
final class PositionPermissionController
{
    ...
    public function update(Request $request, int|string $id): Response
    {
        try {
            ...
            return Response::success('Position permissions updated.', $this->service->replace(
                $id,
                (array) ($request->input('permissions') ?? $request->all()),
                $this->authorizationService->effectivePermissionCodes($user)
            ));
        }
