...
final class AuthorizationMiddleware
{
    ...
    public function handle(Request $request, string $permission, callable $next, ?callable $targetResolver = null): Response
    {
        $user = $request->attribute('auth.user');
        if (!is_array($user)) { return Response::error('unauthenticated', 'Authentication is required.', 401); }
        $request->setAttribute('auth.scope.organization_ids', $this->service->scopeFor($user));
        $target = $targetResolver === null ? null : $targetResolver($request);
        $failure = $this->service->authorize($user, $permission, $target);
