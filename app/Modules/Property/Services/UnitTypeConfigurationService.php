...
final class UnitTypeConfigurationService {
 ...
 public function createBlankDraft(int|string $unit, int|string|null $actor=null):array{return $this->createDraft($unit,false,$actor);}
 public function cloneActiveDraft(int|string $unit,int|string|null $actor=null):array{return $this->createDraft($unit,true,$actor);}
 private function createDraft(int|string $unit,bool $clone,int|string|null $actor):array{return $this->db->transaction(function()use($unit,$clone,$actor){$u=$this->need($this->units->findUnitTypeForUpdate($unit),'CATALOG_ITEM_INACTIVE');if($u['status']!=='active')$this->fail('CATALOG_ITEM_INACTIVE');if($this->configs->findDraftConfigurationForUpdate($u['id']))$this->fail('CONFIGURATION_DRAFT_ALREADY_EXISTS');$src=$clone?$this->configs->findActiveConfigurationForUnitTypeForUpdate($u['id']):null;if($clone&&!$src)$this->fail('NO_ACTIVE_CONFIGURATION');$n=$u['last_allocated_configuration_version'];if($n>=4294967295)$this->fail('CONFIGURATION_NOT_ACTIVATABLE');$n++;$this->units->updateLastAllocatedConfigurationVersion($u['id'],$n);$d=$this->configs->createConfigurationVersion(['ulid'=>$this->ulids->generate(),'unit_type_id'=>$u['id'],'version_number'=>$n,'status'=>'draft','provenance'=>'SYSTEM_ADMIN','created_by_user_id'=>$actor,'updated_by_user_id'=>$actor]);if($src){$this->validate($src['id']);foreach($this->configs->listMeasurementRules($src['id'])as$r)$this->configs->createMeasurementRule($d['id'],['measurement_definition_id'=>$r['measurement_definition_id'],'requirement'=>$r['requirement'],'is_primary'=>$r['is_primary'],'sort_order'=>$r['sort_order'],'created_by_user_id'=>$actor,'updated_by_user_id'=>$actor]);foreach($this->configs->listAttributeRules($src['id'])as$r)$this->configs->createAttributeRule($d['id'],['attribute_definition_id'=>$r['attribute_definition_id'],'requirement'=>$r['requirement'],'sort_order'=>$r['sort_order'],'created_by_user_id'=>$actor,'updated_by_user_id'=>$actor]);}return $d;});}
